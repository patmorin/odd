...
\begin{proof}[Proof of \cref{new_result}]
  ...
  \end{compactitem}
  Note that, for any edge $vw$ of $G$, $v \in C_w$ or $w\in C_v$.  Therefore, (\ding{96}) implies that the colouring $\varphi$ is a proper colouring of $G$.
  ...
  \begin{itemize}
    \item To guarantee that $\varphi$ satisfies (\ding{96}), we avoid colours in the set $X:= \{\varphi(w): w\in C_{v}\}$.
    By definition, if we choose $\varphi(v)\not\in X$ then the resulting colouring will satisfy (\ding{96}).  In particular, choosing $\varphi(v)\not\in X$ ensures that $v$ satisfies (\ding{96}). Furthermore, by the maximality of $(i,j)$, $v\not\in C_w$ for any $w\in V(G)$ so colouring $v$ cannot violate (\ding{96}) for any $w\in V(G)\setminus\{v\}$.

    Furthermore, if $v$ is not an isolated vertex of $G$ then (\ding{96}) ensures that some colour occurs exactly once in $N_G(v)$. Thus we need only ensure that we choose $\varphi(v)$ so that some colour appears an odd number of times in $N_G(w)$ for each $w\in N_G(v)$, which is what we do next.

    \item To guarantee that $\varphi$ is odd, we consider each vertex $w\in N_{G}(v)$.  If there is exactly one colour $\alpha\in\{1,\ldots,6t+3\}$ that occurs an odd number of times in $N_{G'}(w)$, then we define $Y_{w} := \{\alpha\}$; otherwise we define $Y_{w}:=\emptyset$. Now let $Y:=\bigcup_{w\in N_{G}(v)} Y_{w}$.
    ...
    \end{itemize}
  \end{itemize}
  Therefore, by choosing $\varphi(v)\not\in X\cup Y$ we obtain an odd colouring of $G$ that satisifies (\ding{96}).  The sets $X$ and $Y$ each have size at most $3t+1$, so $|X\cup Y|\le 6t+2$.  Therefore we can always find a value $\varphi(v)\in \{1,\ldots,6t+3\}\setminus(X\cup Y)$ that completes the colouring of $G$.
\end{proof}
...
% \cref{new_result2} has an interesting consequence.  The \emph{growth rate} of a graph $G$ is the minimum $\rho$ such that every ball of radius $r>1$ in $G$ contains at most $r^\rho$ vertices.  \citet{krauthgamer.lee:intrinsic} show that, if $G$ has growth rate $\rho$, then $G$ is a subgraph of the $O(c\log c)$-fold strong product $X:=\mathlarger{\mathlarger{\boxtimes}}_{i=1}^{O(c\log c)} P$ of a path with itself.  The graph $X$ has maximum-degree
