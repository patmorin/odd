...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \end{itemize}
  \end{itemize}
  Therefore, by choosing $\varphi(v)\not\in X\cup Y$ we obtain an odd colouring of $G$ that satisifies (\ding{96}).  Since $|X\cup Y|\le |X|+|Y|\le 8t+3 < |\ell(v)|$, there exists some $\varphi(v)\in\ell(v)\setminus(X\cup Y)$ that completes the colouring of $G$.
\end{proof}
